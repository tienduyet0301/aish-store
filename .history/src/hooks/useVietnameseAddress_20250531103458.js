...
export const useVietnameseAddress = () => {
  ...
  useEffect(() => {
    ...
    const fetchDistricts = async () => {
      ...
      try {
        const response = await fetch(`https://provinces.open-api.vn/api/p/${selectedProvince}?depth=2`, { signal });
        const data = await response.json();
        setDistricts(data.districts);
        setSelectedProvinceName(data.name);
      } catch (error) {
        if (error.name === 'AbortError') {
          console.log('Districts fetch aborted');
        } else {
  ...
  useEffect(() => {
    ...
    const fetchWards = async () => {
      ...
      try {
        const response = await fetch(`https://provinces.open-api.vn/api/d/${selectedDistrict}?depth=2`, { signal });
        const data = await response.json();
        setWards(data.wards);
        ...
        if (error.name === 'AbortError') {
          console.log('Wards fetch aborted');
        } else {
