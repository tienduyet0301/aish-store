...
export default function ShippingPage({
  ...
  searchParams,
  updateShippingInfo,
}: ShippingPageProps & { updateShippingInfo?: (fee: number, province: string) => void }) {
  ...
  const { shippingFee, setShippingFee, calculateShippingFee, formatShippingFee } = useCheckout();
  ...
  useEffect(() => {
    if (selectedProvinceName) {
      const fee = calculateShippingFee(selectedProvinceName);
      setShippingFee(fee);
      // Update parent component
      if (updateShippingInfo) {
        updateShippingInfo(fee, selectedProvinceName);
      }
    }
  }, [selectedProvinceName, calculateShippingFee, setShippingFee, updateShippingInfo]);
  ...
  const handleCompleteOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const subtotal = cartItems.reduce((total: number, item: OrderItem) => total + (typeof item.price === 'string' ? parseFloat(item.price) : item.price) * item.quantity, 0);
      const promoAmount = localStorage.getItem('promoAmount') ? parseFloat(localStorage.getItem('promoAmount')!) : 0;
      ...
      const details: OrderData = {
        ...
        total: subtotal - promoAmount + shippingFee,
      };

      console.log('Order details before showing confirmation modal:', details);
              ...
              {selectedProvinceName && (
                <div style={{
                  ...
                  <div style={{
                    ...
                    <span style={{ fontWeight: "600" }}>{formatShippingFee(shippingFee)}</span>
                  </div>
                  <div style={{
                    fontSize: "0.8em",
                    color: "#666"
