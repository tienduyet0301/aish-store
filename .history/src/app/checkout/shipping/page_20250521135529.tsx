...
export default function ShippingPage({
  ...
  const handleCompleteOrder = async (e: React.FormEvent) => {
    ...
    // Log the userEmail just before checking
    console.log('User email for order:', userEmail);

    // Kiểm tra xem email có tồn tại không
    if (!userEmail) {
      alert("Please provide your email address or log in.");
    ...
    try {
      ...
      const details: OrderData = {
        ...
        promoCode: undefined,
        promoAmount: 0,
        total: cartItems.reduce((total: number, item: OrderItem) => total + (typeof item.price === 'string' ? parseFloat(item.price) : item.price) * item.quantity, 0),
      };
