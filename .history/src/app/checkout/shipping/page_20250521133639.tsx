  ...
  const handleCompleteOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const details: OrderData = {
        ...
        subtotal: cartItems.reduce((total, item) => total + (typeof item.price === 'string' ? parseFloat(item.price) : item.price) * item.quantity, 0),
        promoCode: undefined,
        promoAmount: 0,
        total: cartItems.reduce((total, item) => total + (typeof item.price === 'string' ? parseFloat(item.price) : item.price) * item.quantity, 0),
      };
