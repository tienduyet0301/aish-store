  ...
  const handleCompleteOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const details: OrderData = {
        ...
        subtotal: cartItems.reduce((total: number, item: OrderItem) => total + (typeof item.price === 'string' ? parseFloat(item.price) : item.price) * item.quantity, 0),
        ...
        total: cartItems.reduce((total: number, item: OrderItem) => total + (typeof item.price === 'string' ? parseFloat(item.price) : item.price) * item.quantity, 0) - (localStorage.getItem('promoAmount') ? parseFloat(localStorage.getItem('promoAmount')!) : 0),
      };
