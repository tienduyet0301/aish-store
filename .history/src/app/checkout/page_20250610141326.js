  ...
  const handleApplyPromoCode = async () => {
    ...
    try {
      const response = await fetch("/api/apply-promo", {
        ...
        body: JSON.stringify({
          code: promoCode.trim(),
          totalAmount: order.total, // Ensure this is the total amount before discount
          cartItems: cartItems.map(item => ({ productId: item.id, price: item.price, quantity: item.quantity }))
        }),
      });
      ...
      if (data.ok) {
        ...
        setPromoMessage(data.message || "Mã giảm giá không hợp lệ");
        setPromoAmount(0);
        setIsPromoApplied(false);
      }
    } catch (error) {
