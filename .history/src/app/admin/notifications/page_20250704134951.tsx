...
export default function NotificationsAdmin() {
  ...
  const [newPromoMinOrderAmount, setNewPromoMinOrderAmount] = useState(""); // State cho giá trị đơn hàng tối thiểu
  const [heroTitle, setHeroTitle] = useState("");
  const [heroContent, setHeroContent] = useState("");
  const [heroExpiry, setHeroExpiry] = useState("");
  ...
  const handleAddPromoCode = async (e: React.FormEvent) => {
    ...
    const promoMinOrderAmountNumeric = newPromoMinOrderAmount.trim() ? parseFloat(newPromoMinOrderAmount.replace(/[^0-9.-]+/g, '')) : 0;
    if (newPromoMinOrderAmount.trim() && (isNaN(promoMinOrderAmountNumeric) || promoMinOrderAmountNumeric < 0)) {
      setMessage("Giá trị đơn hàng tối thiểu không hợp lệ");
      return;
    }
    ...
    try {
      const response = await fetch("/api/admin/notifications", {
        ...
        body: JSON.stringify({
          ...
          minOrderAmount: promoMinOrderAmountNumeric
        }),
      });
  ...
  const handleHeroSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.ok) {
        setMessage("Thông báo nằm yên đã được cập nhật!");
        setTimeout(() => setMessage(""), 3000);
      }
    } catch (error) {
      setMessage("Có lỗi xảy ra khi cập nhật thông báo nằm yên");
    }
  };
    ...
    <div className="min-h-screen bg-white">
      <div className="w-full">
        ...
        <div className="p-8">
          ...
          <div className="border border-gray-200 mb-6 mt-6">
            <div className="border-b border-gray-200 px-6 py-4">
              <h2 className="text-lg font-bold text-black">Thông báo nằm yên (hero)</h2>
            </div>
            <div className="p-6">
              <form onSubmit={handleHeroSubmit}>
                ...
                <div className="mb-4">
                  <label className="block text-sm font-medium text-black mb-1">Nội dung</label>
                  <input
                    type="text"
                    value={heroContent}
                    onChange={(e) => setHeroContent(e.target.value)}
                    className="w-full border border-gray-200 px-3 py-2 text-sm text-black focus:outline-none focus:border-gray-400"
                    placeholder="VD: get 20% off if you spend 120$ or more!"
                  />
                </div>
                <div className="mb-4">
                  ...
                  <input
                    type="datetime-local"
                    value={heroExpiry}
                    onChange={(e) => setHeroExpiry(e.target.value)}
                    className="w-full border border-gray-200 px-3 py-2 text-sm text-black focus:outline-none focus:border-gray-400"
                  />
                </div>
                <button
          ...
          <div className="border border-gray-200">
            ...
            <div className="p-6">
              <form onSubmit={handleAddPromoCode} className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div>
                    <label className="block text-sm font-medium text-black mb-1">Giá trị đơn hàng tối thiểu (VND)</label>
                    <input
                      type="number"
                      value={newPromoMinOrderAmount}
                      onChange={(e) => setNewPromoMinOrderAmount(e.target.value)}
                      className="w-full border border-gray-200 px-3 py-2 text-sm text-black focus:outline-none focus:border-gray-400"
