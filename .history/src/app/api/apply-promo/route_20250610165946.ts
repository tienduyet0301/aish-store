...
export async function POST(req: Request) {
  try {
    ...
    if (promoCodeDoc.scope === 'selected') {
      ...
      const allowedProducts = cartItems.filter(item => 
        promoCodeDoc.selectedProducts.some(p => p.productId === item.productId)
      );
      ...
      const allowedTotalAmount = allowedProducts.reduce((total, item) => total + item.price * item.quantity, 0);
