...
export async function POST(req: Request) {
  try {
    ...
    if (promoCodeDoc.scope === 'selected') {
      ...
      console.log('selectedProducts:', promoCodeDoc.selectedProducts);
      console.log('cartItems:', cartItems.map((i: { productId: string, id?: string }) => ({ productId: i.productId, id: i.id })));

      // Kiểm tra xem có sản phẩm nào trong giỏ hàng được phép áp dụng mã không
      const allowedProducts = cartItems.filter((item: { productId: string, id?: string }) => 
        promoCodeDoc.selectedProducts.map(String).includes(String(item.productId)) ||
        promoCodeDoc.selectedProducts.map(String).includes(String(item.id))
      );
      ...
      if (promoCodeDoc.minOrderAmount && allowedTotalAmount < promoCodeDoc.minOrderAmount) {
        return NextResponse.json(
          { ok: false, message: `Đơn hàng của bạn chưa đủ giá trị tối thiểu (${promoCodeDoc.minOrderAmount.toLocaleString('vi-VN')} VND) để sử dụng mã giảm giá này` },
          { status: 400 }
        );
      }
    ...
    // Kiểm tra giá trị đơn hàng tối thiểu
    if (promoCodeDoc.minOrderAmount && totalAmount < promoCodeDoc.minOrderAmount) {
      return NextResponse.json(
        { ok: false, message: `Đơn hàng của bạn chưa đủ giá trị tối thiểu (${promoCodeDoc.minOrderAmount.toLocaleString('vi-VN')} VND) để sử dụng mã giảm giá này` },
        { status: 400 }
      );
