...
function HeroNotificationOverlay() {
  ...
  return (
    <div style={{position:'absolute',left:0,right:0,bottom:'10%',zIndex:10,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',pointerEvents:'none'}}>
      <h1 style={{fontSize:'3rem',fontWeight:700,color:'#F5E6C8',textShadow:'0 2px 8px #000'}}>{hero.title}</h1>
      <div style={{fontSize:'1.2rem',color:'#F5E6C8',marginTop:8,textShadow:'0 2px 8px #000'}}>{hero.content}</div>
      {hero.expiry && (
        <div style={{fontSize:'2rem',marginTop:16,color:'#F5E6C8',letterSpacing:2,textShadow:'0 2px 8px #000'}}>{countdown}</div>
      )}
    </div>
  );
}
