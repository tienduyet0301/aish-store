...
function HeroNotificationOverlay() {
  ...
  return (
    <div style={{position:'absolute',left:0,right:0,bottom:'10%',zIndex:10,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',pointerEvents:'none'}}>
      <h1 style={{fontSize:'2.3rem',fontWeight:700,color:'#fff',textShadow:'0 2px 8px #000'}}>{hero.title}</h1>
      <div style={{fontSize:'1rem',color:'#fff',marginTop:6,textShadow:'0 2px 8px #000'}}>{hero.content}</div>
      {hero.expiry && countdown && (
        <div style={{display:'flex',gap:18,marginTop:16,alignItems:'center',justifyContent:'center'}}>
          {[{label:'DAYS',value:countdown.days},{label:'HOURS',value:countdown.hours},{label:'MINUTES',value:countdown.minutes},{label:'SECONDS',value:countdown.seconds}].map((item,idx,arr)=>(
            <React.Fragment key={item.label}>
              <div style={{display:'flex',flexDirection:'column',alignItems:'center',minWidth:40}}>
                <span style={{fontSize:'1.7rem',fontWeight:600,color:'#fff',lineHeight:1}}>{item.value.toString().padStart(2,'0')}</span>
                <span style={{fontSize:'0.75rem',color:'#fff',marginTop:2,letterSpacing:2}}>{item.label}</span>
              </div>
              {idx<arr.length-1 && <div style={{height:28,width:1,background:'#fff',opacity:0.5,margin:'0 7px'}}/>}
            </React.Fragment>
          ))}
        </div>
      )}
