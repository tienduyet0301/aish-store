...
export default function TowelPage() {
  ...
  const [activeButton, setActiveButton] = useState("towel");
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        if (data.ok) {
          const towelProducts = data.products
            .filter(product => product.category === "TOWEL")
            .map(product => ({
              ...product,
