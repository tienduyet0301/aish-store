...
export default function SocksPage() {
  ...
  const [activeButton, setActiveButton] = useState("socks");
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        if (data.ok) {
          const socksProducts = data.products
            .filter(product => product.category === "SOCKS")
            .map(product => ({
              ...product,
        ...
        <div className="px-2 sm:px-4 md:px-6 lg:px-8">
          <div className="flex justify-between items-center relative">
            <div className="inline-block">
              <button
                ref={buttonRefs.socks}
                onMouseEnter={() => handleButtonHover("socks")}
